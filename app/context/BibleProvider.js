...
export function BibleProvider({ children }) {
  ...
  const increaseLineHeight = () => setLineHeight((prev) => prev + LINE_HEIGHT_STEP);
  const decreaseLineHeight = () => setLineHeight((prev) => prev - LINE_HEIGHT_STEP);
  ...
  useEffect(() => {
    ...
    saveSettings();
  }, [selectedTranslation, selectedBook, selectedChapter, fontSize, lineHeight, loading]);
  ...
  useEffect(() => {
    ...
    fetch(`https://bolls.life/get-books/${selectedTranslation}/`)
      ...
      .then((data) => {
        const filteredBooks = data.filter((b) => PROTESTANT_BOOKS.includes(b.name));
        ...
        if (filteredBooks.length > 0) {
          ...
          if(initialState) {
              const foundBook = filteredBooks.find(b => b.bookid === initialState.selectedBookId);
              if (foundBook) {
                bookToSet = foundBook;
                chapterToSet = initialState.selectedChapter > foundBook.chapters ? foundBook.chapters : initialState.selectedChapter;
              } else {
                  bookToSet = filteredBooks[0];
                  chapterToSet = 1;
              }
              setInitialState(null); // Clear initial state after applying it
          } else if (!selectedBook || !books.some(b => b.bookid === selectedBook.bookid)) {
